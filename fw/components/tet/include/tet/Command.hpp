...
struct Command {
    ...
    consteval auto encode() const {
        return indent<t_indent> + "\"" + identifier + "\": {\n" + indent<t_indent + 1> + "\"description\": \"" + description + "\",\n" + indent<t_indent + 1> + "\"arguments\": {\n" + encodeMultiple<t_indent + 2, 0>(arguments) + indent<t_indent + 1> + "}\n" + indent<t_indent> + "}";
    }
};

template <HW::State State, std::size_t t_identifierSize, std::size_t t_descriptionSize, typename... Args>
Command(const char (&identifier)[t_identifierSize],
